...
export class profileStatPage {
    ...
    constructor(fb: FormBuilder,public platform: Platform,public navCtrl: NavController,private _http: Http ,public modalCtrl: ModalController ,sanitizer:DomSanitizationService ,public alertCtrl: AlertController,public actionSheetCtrl: ActionSheetController) {
        ...
        this.local.get('userinfo').then((value) => {
            ...
            if(value!=null) {
                ...
                /*****************Get User Stat[start]**********************/
                this.getFromdb('profileStatdata2');
                    ...
                    this._http.post(link5, data5)
                        .subscribe(res => {
                            if (res.json() == null) {
                                ...
                                this.opendatase('profileStatdata2');
                                //this.opendatase('profileStatusdata');
                            }
                        }, error => {
                            console.log("Oooops!");
    ...
    tableinsert(tbl){
        ...
        if(tbl == 'profileStatdata2'){
            data = JSON.stringify(this.statdata2);
        }
            ...
            db.executeSql("SELECT * FROM "+tbl, {}).then((data) => {
                ...
                if(data.rows.length){
                    ...
                    if(tbl == 'profileStatdata2'){
                        ...
                        this.statdata2 = JSON.parse(redata);
                        this.statcount = redata.length;
                    }
                    if(tbl == 'profileBanner1data'){
